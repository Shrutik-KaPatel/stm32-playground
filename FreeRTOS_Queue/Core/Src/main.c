...
int main(void)
{
  ...
  dataQueueHandle = osMessageQueueNew (5, sizeof(uint32_t), &dataQueue_attributes);
...
void StartProducerTask(void *argument)
{
  ...
  {
	  counter++;
	  osMessageQueuePut(dataQueueHandle, &counter, 0, osWaitForever);
...
void StartConsumerTask(void *argument)
{
  ...
  {
	  uint32_t received;
	  osMessageQueueGet(dataQueueHandle, &received, 0, osWaitForever);
