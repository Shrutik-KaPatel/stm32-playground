...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define BUTTON_FLAG   (1 << 0)   // bit 0 = 0x01
#define SENSOR_FLAG   (1 << 1)   // bit 1 = 0x02
#define TIMER_FLAG    (1 << 2)   // bit 2 = 0x04
/* USER CODE END PD */
...
/* Definitions for sysEvents */
osEventFlagsId_t sysEventsHandle;
const osEventFlagsAttr_t sysEvents_attributes = {
  .name = "sysEvents"
};
/* USER CODE BEGIN PV */
...
int main(void)
{
  ...
  /* Create the event(s) */
  /* creation of sysEvents */
  sysEventsHandle = osEventFlagsNew(&sysEvents_attributes);
...
void StartButtonTask(void *argument)
{
  ...
  /* Infinite loop */
	...
	for(;;)
  {
		uint32_t state = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0);
			      if(state != lastState)
			      {
			    	osEventFlagsSet(sysEventsHandle, BUTTON_FLAG);
			        lastState = state;
			      }
			      osDelay(200);
  }
...
void StartSensorTask(void *argument)
{
	...
	for(;;)
  {
		uint32_t state = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1);
					      if(state != lastState)
					      {
					    	osEventFlagsSet(sysEventsHandle, SENSOR_FLAG);
					        lastState = state;
					      }
					      osDelay(50);
  }
...
void StartTimerTask(void *argument)
{
  ...
  {
    osDelay(3000);
    osEventFlagsSet(sysEventsHandle, TIMER_FLAG);
  }
...
void StarTactionTask(void *argument)
{
  ...
  for(;;)
  {
	  // Wait for ALL flags (blocks until all set)
	osEventFlagsWait(sysEventsHandle, BUTTON_FLAG | SENSOR_FLAG | TIMER_FLAG, osFlagsWaitAll, osWaitForever);
