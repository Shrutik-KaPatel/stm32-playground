...
int main(void)
{
  ...
  LIS3_Init(&hlis);                                                // CTRL_REG4 = 0x77, ODR 400Hz
  LIS3_WriteReg(&hlis, LIS3_CTRL_REG3, LIS3_CTRL_REG3_DRDY_INT1);  // CTRL_REG3 = 0xE8, DRDY on INT1
  ...
  while (1)
  {
	  ...
	  {
	      data_ready = 0;
	      LIS3_ReadXYZ(&hlis, &data);

	      fft_buffer[fft_index++] = (float32_t)data.x;

	      if (fft_index >= FFT_SIZE)
	      {
	          fft_index = 0;
	          arm_rfft_fast_f32(&fft_instance, fft_buffer, fft_output, 0);
