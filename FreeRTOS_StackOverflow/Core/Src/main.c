...
/* Definitions for overflowTask */
osThreadId_t overflowTaskHandle;
const osThreadAttr_t overflowTask_attributes = {
  .name = "overflowTask",
  .stack_size = 512 * 4,
  .priority = (osPriority_t) osPriorityNormal,
};
...
void StartOverflowTask(void *argument);
...
int main(void)
{
  ...
  /* creation of overflowTask */
  overflowTaskHandle = osThreadNew(StartOverflowTask, NULL, &overflowTask_attributes);
...
void StartOverflowTask(void *argument)
{
  ...
  for(;;)
  {
	  uint8_t bigBuffer[599];  // 600 bytes > 512 byte stack
	      bigBuffer[0] = 0;        // force compiler to actually allocate it
	      printf("overflowTask running\r\n");
	      osDelay(500);
  }
