...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
// Custom type for timer status
typedef uint32_t TIM_TIMER_STATUS;

// typedef for function pointer clean nickname!
typedef void (*TIM_TIMER_CALLBACK)(TIM_TIMER_STATUS status, uintptr_t context);

// Struct bundling callback + context together
typedef struct
{
    TIM_TIMER_CALLBACK callback;
    uintptr_t context;
} TIM_TIMER_CALLBACK_OBJ;

// Create the actual callback object
TIM_TIMER_CALLBACK_OBJ TIM3_CallbackObject;

// Registration function fills in the struct
...
// callback execution — debounce + call LED function
...
int main(void)
{
  ...
  TIM3_TimerCallbackRegister(callback_execution, 0);
  /* USER CODE END 2 */
