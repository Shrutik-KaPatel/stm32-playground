...
/* Definitions for laserQueue */
osMessageQueueId_t laserQueueHandle;
const osMessageQueueAttr_t laserQueue_attributes = {
  .name = "laserQueue"
};
...
int main(void)
{
  ...
  /* Create the queue(s) */
  /* creation of laserQueue */
  laserQueueHandle = osMessageQueueNew (5, sizeof(uint32_t), &laserQueue_attributes);

  /* creation of uartQueue */
  uartQueueHandle = osMessageQueueNew (5, sizeof(uint32_t), &uartQueue_attributes);
...
/* USER CODE BEGIN Header_StartLaserTask */
/**
* @brief Function implementing the laserTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartLaserTask */
void StartLaserTask(void *argument)
{
  /* USER CODE BEGIN StartLaserTask */
  /* Infinite loop */
  ...
  {
	  uint32_t state;
	  osMessageQueueGet(laserQueueHandle, &state, 0, osWaitForever);
	  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_2, state);
	  HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_13);
  }
  /* USER CODE END StartLaserTask */
}
...
void StartUartTask(void *argument)
{
  ...
  {
	  uint32_t state;
	  osMessageQueueGet(uartQueueHandle, &state, 0, osWaitForever);
	  if(state == 1) printf("Touch detected\r\n");
	  else printf("Touch released\r\n");
  }
