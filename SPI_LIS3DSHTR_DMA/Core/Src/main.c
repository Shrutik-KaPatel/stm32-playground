...
uint8_t spi_rx_buf[7];  //1 dummy + 6 data bytes
uint8_t spi_tx_buf[7];  //1 address + 6 dummy bytes
...
int main(void)
{
  ...
  // Prepare TX buffer - address byte + 6 dummies
  spi_tx_buf[0] = LIS3_OUT_X_L | 0x80; // RW=1, start at OUT_X_L
  spi_tx_buf[1] = 0x00;
  spi_tx_buf[2] = 0x00;
  spi_tx_buf[3] = 0x00;
  spi_tx_buf[4] = 0x00;
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (dma_done)
	  {
	      ...
	      int16_t z = (int16_t)(spi_rx_buf[6] << 8 | spi_rx_buf[5]);
